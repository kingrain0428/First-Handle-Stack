...
class InvestTotal extends React.Component{
    ...
    componentDidMount() {
        if (!this.props.location.query.defaultKey) {
            this.setState({
                defaultKey: 1
            })
        } else {
            this.setState({
                defaultKey: this.props.location.query.defaultKey
            })
        }
    }
    ...
    render() {
        return(
            <div id="invest_list">
                ...
                <Tabs onChange={this.handleClick} defaultActiveKey={this.state.defaultKey}>
